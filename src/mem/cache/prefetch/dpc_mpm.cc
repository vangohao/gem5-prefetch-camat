...
#include <stdio.h>
#include "mem/cache/prefetch/dpc_mpm.hh"

#define PREFETCH_DEGREE 2

MPM_DPCPrefetcher::MPM_DPCPrefetcher(const MPM_DPCPrefetcherParams *p) :
    DPCPrefetcher(p)
{}

MPM_DPCPrefetcher*
MPM_DPCPrefetcherParams::create()
{
    return new MPM_DPCPrefetcher(this);
}



void  MPM_DPCPrefetcher::l2_prefetcher_initialize(int cpu_num)
{
...
void MPM_DPCPrefetcher::l2_prefetcher_operate(int cpu_num, unsigned long long int addr, unsigned long long int ip, int cache_hit)
{
...
void MPM_DPCPrefetcher::l2_cache_fill(int cpu_num, unsigned long long int addr, int set, int way, int prefetch, unsigned long long int evicted_addr)
{
  // uncomment this line to see the information available to you when there is a cache fill event
  //printf("0x%llx %d %d %d 0x%llx\n", addr, set, way, prefetch, evicted_addr);
}

void MPM_DPCPrefetcher::l2_prefetcher_heartbeat_stats(int cpu_num)
{
  printf("Prefetcher heartbeat stats\n");
}

void MPM_DPCPrefetcher::l2_prefetcher_warmup_stats(int cpu_num)
{
  printf("Prefetcher warmup complete stats\n\n");
}

void MPM_DPCPrefetcher::l2_prefetcher_final_stats(int cpu_num)
{
